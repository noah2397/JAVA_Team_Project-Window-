...
void main(void) {
	...
	while(1){
		...
		if(input == 0){ //저번달
			...
			int cal_year, cal_month, cal_day, cal_yday, cur_yday, cur_year;
			cur_yday = t->tm_yday;
			...
			if(cal_yday > cur_yday)
				printw("%d days left from today", cal_yday-cur_yday);
			else
